...
var nerTags = map[string]string{
	...
	`\b(?:(?:[Dd]r\.|[Mm]r\.|[Mm]rs\.|[Mm]s\.)\s+)?[A-Z][a-z]+(?:[\s-][A-Z][a-z]+)*(?:\s+(?:[Jj]r\.|[Ss]r\.|I{2,3}|IV))?\b`: "PERSON",
		...
		for pattern, tag := range nerTags {
			...
			if t.NerTag[i] == "" {
				...
				case "NN", "NNS": // Common nouns (singular/plural)
					...
					// ... (Add more specific noun checks based on your domain)
				case "JJ": // Adjectives
					...
					// ... (Add more adjective-related checks as needed)
				}
				// 3. Handle specific cases and unrecognized words:
				if t.NerTag[i] == "" {
					switch token {
					case "webserver":
