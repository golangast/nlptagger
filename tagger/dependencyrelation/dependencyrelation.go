...
func PredictDependencies(t tag.Tag) (tag.Tag, error) {
	...
	if rootIndex != -1 {
		...
		if t.Tokens[rootIndex+1] == "Inventory" && t.NerTag[rootIndex+1] == "OBJECT_NAME" {
			t.Dependencies = append(t.Dependencies, tag.Dependency{Head: rootIndex, Dependent: rootIndex + 1, Relation: "dobj"})
		}
	}
	...
	for i := range t.Tokens {
		...
		if i >= 1 && i < len(t.NerTag) {
			...
			case rootIndex != -1 && t.Tokens[i] == "Inventory" && t.NerTag[i] == "OBJECT_NAME": // Direct object of the root verb (Create Inventory)
				t.Dependencies = append(t.Dependencies, tag.Dependency{Head: rootIndex, Dependent: i, Relation: "dobj"})
			...
			default:
				t.Dependencies = append(t.Dependencies, tag.Dependency{Head: i, Dependent: i, Relation: " "})
			}
		}
	}
	// Add explicit root tag.Dependency if rootIndex is valid
	if rootIndex != -1 {
		t.Dependencies = append(t.Dependencies, tag.Dependency{Head: -1, Dependent: rootIndex, Relation: "root"})
	}
	for i := range t.Tokens {
