...
func NounCheck(t tag.Tag) tag.Tag {
	...
	for i, token := range t.Tokens {
		...
		case i > 2 && i < len(t.Tokens) && token == "Inventory" && i > 0 && t.PosTag[i-1] == "VBN":
			t.PosTag[i] = "NNP"
			break
		...
		case token == "fields":
			t.PosTag[i] = "NNS" // Plural noun for "fields"
			break
		...
		case i < len(t.Tokens) && i > 0 && t.PosTag[i-1] == "VBN" && t.Tokens[i] == "Inventory":
			t.PosTag[i] = "NNP"
			break
		...
		case token == "Inventory" && i > 0 && t.PosTag[i-1] == "VBN":
			t.PosTag[i] = "NNP" // Tag as proper noun
			break
		...
		case isProper && i > 2 && i < len(t.Tokens) && i > 0 && (t.Tokens[i-1] == "a" || t.Tokens[i-1] == "an" || t.Tokens[i-1] == "the"):
